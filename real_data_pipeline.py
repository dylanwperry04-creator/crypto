...
ROLLING_WINDOW = 30
BITCOIN_COMPARE_FEATURE_START = "2022-01-01"
BITCOIN_COMPARE_FEATURE_END = "2023-06-22"
...
def bitcoin_compare_feature_window() -> tuple[pd.Timestamp, pd.Timestamp]:
    return (
        pd.Timestamp(BITCOIN_COMPARE_FEATURE_START, tz="UTC").normalize(),
        pd.Timestamp(BITCOIN_COMPARE_FEATURE_END, tz="UTC").normalize(),
    )


def restrict_frame_to_bitcoin_compare_window(frame: pd.DataFrame, coin: str, mode: str) -> pd.DataFrame:
    if frame.empty or coin != "Bitcoin" or mode not in BITCOIN_MODES:
        return frame.copy()
    start, end = bitcoin_compare_feature_window()
    dates = pd.to_datetime(frame["date"], utc=True, errors="coerce").dt.normalize()
...
def default_metadata_payload() -> dict[str, Any]:
    return {
        ...
        "bitcoin_compare_feature_window": {
            "start": BITCOIN_COMPARE_FEATURE_START,
            "end": BITCOIN_COMPARE_FEATURE_END,
        },
    }
...
def build_feature_matrix() -> pd.DataFrame:
    ...
    if not twitter_daily.empty:
        ...
        twitter_daily["date"] = pd.to_datetime(twitter_daily["date"], utc=True).dt.normalize()
    ...
    compare_start, compare_end = bitcoin_compare_feature_window()
    ...
    for coin in COINS:
        ...
        if coin == "Bitcoin" and twitter_history_available():
            coin_dates = pd.to_datetime(coin_df["date"], utc=True, errors="coerce").dt.normalize()
            overlap_coin_df = coin_df.loc[(coin_dates >= compare_start) & (coin_dates <= compare_end)].copy()
            aligned_coin_df = overlap_coin_df[overlap_coin_df["_twitter_exact_match"].fillna(False)].copy()
...
def train_compare_models() -> list[TrainingOutcome]:
    ...
    for coin in COINS:
        ...
        for mode in coin_modes:
            ...
            frame = restrict_frame_to_bitcoin_compare_window(frame, coin, mode)
            if frame.empty:
                continue

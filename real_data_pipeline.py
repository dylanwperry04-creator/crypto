...
def read_twitter_raw() -> pd.DataFrame:
    ...
    if "sentiment_label" not in df.columns:
        df["sentiment_label"] = np.where(df["sentiment_score"] >= 0, "positive", "negative")
...
def build_feature_matrix() -> pd.DataFrame:
    ...
    # Build per-mode rows with explicit mode column
    ...
    for coin in COINS:
        ...
        price_mode = coin_df[base_cols + ["close"] + price_only_cols].copy()
        ...
        if coin == "Bitcoin" and twitter_history_available():
            ...
            tw_only = coin_df[base_cols + ["close"] + twitter_cols].copy()
            tw_only["mode"] = TWITTER_ONLY_MODE
            feature_sets.append(tw_only)
            combined = coin_df[base_cols + ["close"] + price_only_cols + twitter_cols].copy()
    ...
    for coin in COINS:
        ...
        for mode in coin_modes:
            ...
            for candidate in candidate_models(y_train):
                ...
                # Refit selected hyperparameters on train+validation and evaluate on test
            ...
            for row in candidate_rows:
                if row.get("model_family") == best_candidate["model_family"] and row.get("status") == "trained":
                    row["selected_for_production"] = True
            
            # Persist winning model + feature list
            ...
            # Selected backtests (test split only)
            ...
            # Strip non-serialisable internals and add to results rows
            for row in candidate_rows:
...
def build_live_feature_row(coin: str, mode: str) -> pd.Series | None:
    # rebuild features from latest locally stored history after sync
    feature_matrix = build_feature_matrix()
    if feature_matrix.empty:
        return None
    frame = read_feature_matrix(coin, mode)
    if frame.empty:
        return None
    # latest unresolved row uses the most recent feature date with no known target yet

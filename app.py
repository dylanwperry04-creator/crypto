...
def mode_feature_history(coin: str, mode: str) -> pd.DataFrame:
    frame = read_feature_matrix(coin, mode)
    if frame.empty:
        return frame
    frame = frame.copy()
    frame["date"] = frame["date"].apply(normalize_date_str)
    if "target_date" in frame.columns:
        frame["target_date"] = frame["target_date"].apply(normalize_date_str)
...
def mode_status(coin: str, mode: str) -> dict[str, Any]:
    ...
    history_df = mode_feature_history(coin, mode)
    price_df = read_price_history(coin)

    history_available = bool(model is not None and features and not history_df.empty)

    disabled_reason = None
    if model is None or not model_path.exists():
        disabled_reason = f"Selected model artifact missing at {model_path.relative_to(BASE_DIR)}. Run the offline training pipeline first."
    elif not features:
        disabled_reason = f"Feature list missing at {feature_path.relative_to(BASE_DIR)}. Run the offline training pipeline first."
    elif history_df.empty:
        if coin == "Bitcoin" and mode in {"twitter_only", "combined"}:
            disabled_reason = "Historical Twitter-dependent modes are unavailable because no trained Twitter artifacts were produced. Ensure the real Twitter CSV is loaded into data/raw_twitter_sentiment.csv, then rerun build_features.py and train_compare_models.py."
        else:
            disabled_reason = "No stored historical feature rows are available yet. Run the offline feature pipeline first."
    elif price_df.empty:
    ...
    if mode in {"twitter_only", "combined"} and not TWITTER_LIVE_AVAILABLE:
        live_valid = False
        if not disabled_reason and history_available:
            disabled_reason = "Historical-only mode: live Twitter ingestion is not configured, so this mode is disabled for live scoring."

    date_min = None if history_df.empty else str(history_df["date"].dropna().min())
    date_max = None if history_df.empty else str(history_df["date"].dropna().max())

    return {
        "coin": coin,
        "mode": mode,
        "label": MODE_LABELS[mode],
        "backtest_available": history_available,
        "strict_backtest_available": bool(selected and model is not None and features and not backtest_df.empty),
...
def validate_mode_request(coin: str, mode: str, for_live: bool, date_str: str | None = None) -> dict[str, Any]:
    ...
    if not for_live and date_str and status["date_min"] and status["date_max"] and not (status["date_min"] <= date_str <= status["date_max"]):
        return {"valid": False, "reason": f"Historical data for {coin}/{mode} is available only from {status['date_min']} to {status['date_max']}."}
...
def performance_summary_for_mode(coin: str, mode: str) -> dict[str, Any]:
    ...
    if not results.empty and {"selected_for_production", "status"}.issubset(results.columns):
        mask = (
            results["selected_for_production"].astype(str).str.lower().isin(["true", "1", "yes"])
            & results["status"].astype(str).eq("trained")
        )
        selected = results[mask].copy()
    ...
    return {
        ...
        "label": MODE_LABELS[mode],
        "available": True,
        "pipeline": row.get("model_family"),
        "threshold": None if pd.isna(row.get("threshold")) else round(float(row.get("threshold")), 6),
        "f1": None if pd.isna(row.get("test_f1_class1")) else round(float(row.get("test_f1_class1")), 4),
        "accuracy": round(float((frame["was_correct"].astype(int)).mean()), 4),
        "pr_auc": None if pd.isna(row.get("test_pr_auc")) else round(float(row.get("test_pr_auc")), 4),
        "precision": None if pd.isna(row.get("test_precision")) else round(float(row.get("test_precision")), 4),
        "recall": None if pd.isna(row.get("test_recall")) else round(float(row.get("test_recall")), 4),
        "balanced_accuracy": None if pd.isna(row.get("test_balanced_accuracy")) else round(float(row.get("test_balanced_accuracy")), 4),
        "roc_auc": None if pd.isna(row.get("test_roc_auc")) else round(float(row.get("test_roc_auc")), 4),
        "rows": int(len(frame)),
...
def historical_model_lookup(coin: str, mode: str, date_str: str) -> Optional[dict[str, Any]]:
    model_path = model_artifact_path(coin, mode)
    feature_path = feature_artifact_path(coin, mode)
    model = load_model(model_path)
    features = load_feature_list(feature_path)
    ...
    frame = mode_feature_history(coin, mode)
    if frame.empty:
        return None
    row_match = frame[frame["date"] == date_str]
    if row_match.empty:
        return None
    row = row_match.iloc[0]
    selected = active_model_record(coin, mode) or {}
    ...
    target_value = pd.to_numeric(pd.Series([row.get("target")]), errors="coerce").iloc[0]
    if pd.notna(target_value):
        actual_class = int(target_value)
        actual_label = "UP" if actual_class == 1 else "DOWN"
        result = "Correct" if actual_class == predicted_class else "Wrong"
        status = "Resolved"

    target_date = normalize_date_str(row.get("target_date"))
    if target_date is None and date_str is not None:
        target_date = (pd.Timestamp(date_str) + pd.Timedelta(days=1)).strftime("%Y-%m-%d")

    close_price = pd.to_numeric(pd.Series([row.get("close")]), errors="coerce").iloc[0]
    next_close = pd.to_numeric(pd.Series([row.get("next_close")]), errors="coerce").iloc[0]

    return {
        "context": "historical_model_replay",
        "coin": coin,
        "mode": mode,
        ...
        "result": result,
        "close_price": None if pd.isna(close_price) else round(float(close_price), 6),
        "target_close": None if pd.isna(next_close) else round(float(next_close), 6),
        "pipeline": selected.get("selected_model_family"),
        "audit": {
            "model_artifact": model_path.name if model_path.exists() else None,
            "model_version": file_version(model_path),
            "feature_artifact": feature_path.name if feature_path.exists() else None,
            "feature_version": file_version(feature_path),
            "metadata_version": file_version(BEST_MODEL_METADATA_PATH),
            "evaluation_source": {
                "file": FEATURE_MATRIX_CSV.name,
                "sha256_12": file_version(FEATURE_MATRIX_CSV)["sha256_12"] if file_version(FEATURE_MATRIX_CSV) else None,
                "kind": "saved_feature_matrix_row_scored_on_demand",
            },
        },
    }
    ...
    if not frame.empty:
        ...
        if not row.empty:
            ...
            return {
                ...
                "target_close": round(float(record["target_close"]), 6) if not pd.isna(record.get("target_close")) else None,
                "pipeline": selected.get("selected_model_family") if selected else None,
                "audit": {
                    ...
                    "metadata_version": file_version(BEST_MODEL_METADATA_PATH),
                    "evaluation_source": {
                        "file": BACKTEST_SELECTED_CSV.name,
                        "sha256_12": file_version(BACKTEST_SELECTED_CSV)["sha256_12"] if file_version(BACKTEST_SELECTED_CSV) else None,
                        "kind": "saved_selected_backtest_row",
                    },
                },
            }

    replay = historical_model_lookup(coin, mode, date_str)
...
def create_or_get_live_forecast(coin: str, mode: str) -> dict[str, Any]:
    ...
    feature_date = normalize_date_str(row.get("date"))
    target_date = normalize_date_str(row.get("target_date"))
    if target_date is None and feature_date is not None:
        target_date = (pd.Timestamp(feature_date) + pd.Timedelta(days=1)).strftime("%Y-%m-%d")
    ...
    identity_key = identity_key_for_forecast(coin, mode, feature_date or "na", target_date or "na", model_version or "na", feature_version or "na")
...
def resolve_pending_live_forecasts() -> dict[str, Any]:
    ...
    resolved_count = 0
    errors: list[str] = []
    history = read_price_history().copy()
    if not history.empty:
        history["date_norm"] = history["date"].apply(normalize_date_str)
    with get_db() as conn:
        ...
        for row in rows:
            ...
            try:
                ...
                if coin_df.empty:
                    continue
                target_date = normalize_date_str(row["target_date"])
                target_rows = coin_df[coin_df["date_norm"] == target_date]
...
def backtest_lookup_api():
    ...
    date_str = normalize_date_str(request.args.get("date"))
    if not date_str:
...
def predict_api():
    ...
    date_str = normalize_date_str(args.get("date"))
    explicit_live = args.get("live")
    live = boolish(explicit_live) if explicit_live is not None else not bool(date_str)
    if live:
